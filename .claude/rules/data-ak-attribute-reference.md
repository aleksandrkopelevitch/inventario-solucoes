...
| `data-ak-cati-diagram-pick="<form-id>"` / `-upload="<url>"` | `cati-chat.js` | The C4 slots on the Diagramas tab: the button opens the hidden file input, and the input uploads on `change` (no separate "Enviar" step — that is the one people skip). AS IS / TO BE have neither: they are drawn on the F3 canvas, on their own page |
...
| `data-integration-graph` (on the auto-selected row) | `integration-select.js` → `integration-viz.js` | The whole canvas payload, INCLUDING every endpoint it will call back on (`nodeAddUrl`, `edgeRetargetUrl`, `saveUrl`, `diagramUrl`, …). `integration-viz.js` contains no route of its own, which is what lets the same canvas edit an `Integration` or a `SubmissionDiagram` with no client change at all — see `App\Contracts\ChainCanvas` and `App\Support\ChainGraph` |
