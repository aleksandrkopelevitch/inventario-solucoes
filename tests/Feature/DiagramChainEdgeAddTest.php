...
it('creates a new edge between two existing blocks without adding a node ("modo ligar")', function () {
    ...
    $response = $this->actingAs(edgeAddAdmin())
        ->postJson(route('diagrams.chain.edge.add', $diagram), [
            'from'     => 0,
            'to'       => 2,
            'arrow'    => '->',
            'protocol' => 'rest',
    ...
    expect($diagram->chain['edges'])->toHaveCount(2)
        ->and($diagram->chain['edges'][1])->toBe(['from' => 0, 'to' => 2, 'arrow' => '->', 'protocol' => 'rest'])
        // C is no longer isolated (now has in-degree 1).
        ->and((new ChainLabeler)->isLinear($diagram->chain))->toBeFalse();
});
...
it('returns the index the new edge got in the chain, so the client never infers it', function () {
    ...
    expect($diagram->fresh()->chain['edges'][2])
...
it('rejects connecting a block to itself', function () {
    ...
    expect($diagram->fresh()->chain['edges'])->toBeEmpty();
});
...
it('rejects an edge identical to one already in the chain, but allows a second one that differs', function () {
    ...
    $url = route('diagrams.chain.edge.add', $diagram);

    // Exact duplicate — dragging the same arrow twice out of another port says
    // nothing new and would double-count degrees in SyncDiagramFromChain.
    $response = $this->actingAs(edgeAddAdmin())
    ...
    expect($diagram->fresh()->chain['edges'])->toHaveCount(3);
});
...
it('accepts a free-text protocol not in the Protocol enum', function () {
    ...
    expect($diagram->fresh()->chain['edges'][0]['protocol'])->toBe('carrier-pigeon');
});
