...
it('retargets an edge endpoint to a different block, turning the chain into a free graph', function () {
    ...
    $response = $this->actingAs(edgeRetargetAdmin())
        ->patchJson(route('diagrams.chain.edge.retarget', [$diagram, 1]), [
            'end'  => 'from',
            'node' => 0,
        ])
        ->assertOk()
    ...
    expect($diagram->chain['edges'][1])->toBe(['from' => 0, 'to' => 2, 'arrow' => '->', 'protocol' => null])
        ->and($diagram->participants->pluck('name')->sort()->values()->all())->toBe(['A', 'B', 'C'])
...
it('rejects retargeting an edge endpoint onto its own opposite end (self-loop)', function () {
    ...
    $this->actingAs(edgeRetargetAdmin())
        ->patchJson(route('diagrams.chain.edge.retarget', [$diagram, 0]), [
            'end'  => 'from',
            'node' => 1, // already the 'to' of this edge
        ])
        ->assertStatus(422);

    expect($diagram->fresh()->chain['edges'][0])->toBe(['from' => 0, 'to' => 1, 'arrow' => '->', 'protocol' => null]);
});
...
it('404s for an edge index outside the chain', function () {
    ...
    $this->actingAs(edgeRetargetAdmin())
        ->patchJson(route('diagrams.chain.edge.retarget', [$diagram, 3]), [
            'end'  => 'to',
            'node' => 1,
        ])
        ->assertNotFound();
