...
/**
 * Rebuilds the Diagram's structured fields from the `chain` — the free
 * graph authored entirely by the F3 data-viz (`chain-viz.js`: root
...
{
    public function handle(Diagram $diagram): void
    {
        $chain = $diagram->chain ?? [];
        ...
        // out === 0, being the last node) and silently become the diagram's
        ...
        // `diagrams.protocol` scalar holds a representative value (the
