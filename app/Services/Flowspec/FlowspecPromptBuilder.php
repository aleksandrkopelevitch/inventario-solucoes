...
class FlowspecPromptBuilder
{
    public function systemPrompt(): string
    {
        ...
        3. Steps `choice` roteiam por `when` e `otherwise`; cada condição tem `target` mais UM de três: `jsonPath` (`{target, jsonPath}`), `simple` (`{target, simple}`, ex.: `#{body.RETURNING.STATUS} != '200'`) ou `doubleBraces` (`{target, doubleBraces}`, ex.: `{{ AND(EQUALTO(message.method, "GET"), CONTAINS(message.path, "/rota")) }}`) — use `jsonPath` por padrão, `simple` quando a condição comparar valores e `doubleBraces` quando precisar COMPOR condições ou ler o resultado de um step, que é o que os outros dois não fazem. `target`/`otherwise` referenciam NOMES de branch que PRECISAM existir como chave do `flowSpec`. Para status HTTP use faixa: `$.[?(@.status >= 200 && @.status <= 299)]`.
        ...
        6. Escopos Double Braces válidos: message, global, account, step, metadata, trigger, session, iterators e replica. Funções como UUID(), NOW(), CONCAT() são permitidas. Dentro de um track de for-each, o item da iteração é `{{ iterators.<doubleBracesAlias-do-for-each>.current }}` — é assim que a referência do próprio conector escreve, e era um escopo que o validador recusava. `replica` só vale em realm multi-instância.
