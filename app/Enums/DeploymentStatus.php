 ...
 *
 * `REDEPLOY` was not in that sample and turned up the first time a deploy was
 * actually watched — which is the argument for `Unknown` existing at all.
 * These strings are undocumented (the whole runtime API is), so a status
 * nobody has seen yet must read as "not settled, not healthy" rather than
 * crash a correction loop mid-deploy or, worse, be mistaken for success.
 */
enum DeploymentStatus: string
{
    ...
    public function label(): string
    {
        return match ($this) {
            ...
            self::Redeploying => 'reimplantando',
