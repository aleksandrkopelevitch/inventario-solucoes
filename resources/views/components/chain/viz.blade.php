...
<div data-ak-chain-viz
    ...
    <div data-viz-stage class="relative min-h-0 flex-1">
        <div data-viz-viewport class="ak-viz-viewport">
            <div data-viz-world class="ak-viz-world">
                {{-- The links' pointer targets (`drawEdgeHit()`), on a layer
                     of their OWN. They used to live in the edges svg, which is
                     a sibling of the lanes and comes AFTER them
                     (`rebuildLanes()` inserts each lane at the front of
                     `world`, so a lane paints behind every block) — so a 16px
                     invisible band following every arrow sat on top of the
                     lane resize handles and ate their drag wherever a link
                     crossed a lane's edge, which in a lane drawing is all the
                     time.

                     `z-index: -1` works here because `world` IS a real
                     stacking context (it has a `transform`), so the -1 does not
                     escape behind the component's own background — the trap the
                     lanes' comment describes, which applies to them because
                     they carry no z-index at all. The target is now the LAST
                     option: a block, a lane handle and a lane label all beat
                     it, and it only takes the click where there really is
                     nothing but line. --}}
                ...
                <svg data-viz-edges class="ak-viz-edges" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <style>
                        ...
                        /* Each link's pointer target (`drawEdgeHit()`). It
                           gets `fill: none; stroke: transparent` from the OUTER
                           sheet — which does not travel into the export: the
                           clone carries only this one, and with no rule here
                           the path came out with the browser's default `fill:
                           black`, that is, a black blob following every arrow's
                           route. It is the same trap the comment above
                           describes, and the first new path class created after
    ...
    <style>
            ...
            /* See the comment in the markup. The stroke is `transparent`
               here and `none` as a presentation ATTRIBUTE on each path
               (`drawEdgeHit()`): CSS beats the attribute in the live DOM, and
               in an export clone — which carries none of this sheet — the
               attribute is what is left, instead of the browser's default
               `fill: black`, which already painted a blob over every arrow
               once. */
            .ak-viz-hits {
                position: absolute;
                top: 0;
