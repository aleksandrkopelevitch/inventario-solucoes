<div id="{{ $domId }}" @class(['flex flex-wrap items-center gap-2 transition-[margin] duration-200', 'hidden' => empty($chips)])>
    @foreach ($chips as $chip)
        {{-- Each chip wears its dimension's color family + icon (category = its
             real family, matching the cards) — the active-filters row becomes a
             colorful summary instead of identical pale-green pills. `text-cat-*-ink`
             from $chip['chip'] drives the ink; icon/label dim it, the ✕ inherits it. --}}
        <span data-ak-chip class="animate-chip-pop inline-flex items-center gap-1.5 rounded-full py-1 pl-2.5 pr-1.5 text-xs font-semibold {{ $chip['chip'] }}">
            ...
            <x-forms.button type="button" variant="ghost"
                data-ak-filters-clear="{{ json_encode(['formId' => 'solutions-filter-form', 'field' => $chip['field'], 'url' => route('solutions.index')]) }}"
