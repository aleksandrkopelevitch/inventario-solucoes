...
**The sidebar's `can` gate is now computed once**, in the `$sections` block,
because the rail and the mobile drawer each carried their own copy hard-coding
`Notebook` as the model to ask about. That was invisible while the
knowledge-base settings were the only gated entry — `NotebookPolicy::viewAny`
says yes to every Viewer — so the MCP entry was correctly hidden in the rail and
offered to everybody in the drawer. An item names its own model with `canModel`.
